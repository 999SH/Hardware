...
volatile int *TRISE;//we declare the pointers as volatile so that the compiler does not render the pointers static when optimizing 
volatile int *PORTE;
volatile int *TRISD;
 ...
 *PORTE = 0x00000000; // Set PORTE to 0 so we can see the ticks better
 *TRISE = *TRISE & 0xffffff00; // To mask out the 8 LSBs
...
void labwork( void )
{
  ...
  // Btns 2
  if(btns & 1){             // Mask out the bits that represent 10th seconds (1 = 001)
    mytime = mytime & 0xff0f;  // Take the value from switches, shift to correct position then add (OR) them to mytime.
    mytime = (sw << 4) | mytime;
  }
